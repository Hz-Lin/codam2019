...
/*                                                   +#+                      */
/*   Created: 2020/10/03 17:06:43 by hlin          #+#    #+#                 */
/*   Updated: 2020/10/04 16:44:56 by hlin          ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */

#include "../cub3d.h"

static void		calc_dist(t_game *game, t_view *view)
{
	view->deltadist.x = fabs(1. / view->raydir.x);
	view->deltadist.y = fabs(1. / view->raydir.y);
	if (view->raydir.x < 0)
	{
		set_pos(&view->step, -1, (view->raydir.y < 0) ? -1 : 1);
...
static void		perform_dda(t_game *game, t_view *view)
{
	int	hit;
